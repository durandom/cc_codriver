...
class RbrPacenote:
    ...
    def __str__(self):
        return f'{self.name} - {self.id} - {self.type} - {self.category} - {self.package} - {self.sounds}'
    ...
    def read_ini(self, ini_file = None, recursion = 0, category = None, package = None):
        ...
        for section in config.sections():
            ...
            if section.startswith('PACENOTE') or section.startswith('RANGE'):
                ...
                note = RbrPacenote(name)
                note.type = type
                note.category = category
                note.package = package
                note.id = config.getint(section, 'id', fallback=None)
                if not note.id:
                ...
                if name not in self.pacenotes:
                    ...
                    logging.error('Duplicate pacenote id: %s - %s' % (name, section))
...
class CoDriver:
    ...
    def init_rbr_strings(self):
        file = os.path.join(self.rbr_dir, 'Pacenote', 'language', self.language, 'strings.ini')
        config = configparser.ConfigParser()
        config.read(file)
        self.strings = {}
        for section in config.sections():
            if section == 'STRINGS':
                for english in config.options(section):
                    self.strings[english] = config.get(section, english)
